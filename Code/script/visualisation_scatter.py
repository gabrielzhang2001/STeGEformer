...
def visualise_single_explanation_graph(edge_index, edge_att, num_nodes, pos, vis_path, processed_samples,
                                       edge_threshold):
    ...
    for _, row in df.iterrows():
        if row['latitude'] is not None and row['longitude'] is not None and row['importance_score'] > 0:
            normalized_score = row['importance_score']
            color = get_heatmap_color(normalized_score)
            ...
            folium.CircleMarker(
                ...
                fill=True,
                fillColor=color,
                fillOpacity=0.7 if normalized_score > 0 else 0.1,
                opacity=0.7 if normalized_score > 0 else 0.1,
                popup=f"Node ID: {row['node_id']}<br>Importance: {row['importance_score']:.4f}"
            ).add_to(m)
    ...
    picture_path = os.path.join(vis_path, f"sample_{processed_samples + 1}_scatter.html")
    m.save(picture_path)
...
def visualise_overall_explanation_graph(edge_index, edge_att, num_nodes, pos, vis_path, test_iter, model):
    ...
    picture_path = os.path.join(vis_path, f"overall_scatter.html")
    m.save(picture_path)
